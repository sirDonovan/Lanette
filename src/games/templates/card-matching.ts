...
import { Card, CardType, commands as cardCommands, IPokemonCard, typeColorCodes } from './card';
...
export abstract class CardMatching extends Card {
	...
	getCardPmHtml(card: IPokemonCard): string {
		...
		if (card.id in this.actionCards) {
			...
			html += '<div style="display:inline-block;background-color:' + typeColorCodes['Black']['background-color'] + ';background:' + typeColorCodes['Black']['background'] + ';border-color:' + typeColorCodes['Black']['border-color'] + ';border: 1px solid #a99890;border-radius:3px;width:' + descriptionWidth + ';padding:1px;color:#fff;text-shadow:1px 1px 1px #333;text-transform: uppercase;text-align:center;font-size:8pt"><b>' + description + '</b></div>';
		} else {
